...
class Queue { 
	...
	Queue() {
		size = 0;
		queue = new int[100]; // dynamic memory allocation
	}
	void remove() { 
		if (size == 0) { 
	...
	//your code goes here
    // an constructor takes on item called newdata
	void add(int newdata) { 
		
     // for size i.e not equal to 0 or hundred
	 // increment the size 
	 // queue[size-1] = newdata; =>  The end of the queue adds the data of item passed
